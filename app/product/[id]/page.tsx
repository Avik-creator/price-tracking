    ...
    <div className="w-full min-h-screen bg-[#0B1120] text-white">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-wrap px-6 md:px-20 py-24">
        <h1 className="text-xl md:text-3xl font-bold col-span-1">
          {product.productTitle === substring
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
          <PriceCard
            ...
            label={<BsChevronDoubleUp />}
          />
          <PriceCard
            amount={product.lowestPrice}
            type="Lowest Price"
            currency={product.currency}
            label={<BsChevronDoubleDown />}
          />
          <PriceCard
            amount={product.averagePrice}
            type="Average Price"
            currency={product.currency}
            label={<BsAlignMiddle />}
          />
          <PriceCard
            amount={product.highestPrice}
            type="Highest Price"
            currency={product.currency}
            label={<FaGaugeHigh />}
          />
      ...
      <div className="flex flex-col justify-center items-center my-4">
        ...
        <div className="max-w-[90%] border ">
          <p className="text-slate-300 font-extralight flex items-center justify-center p-4">
            {product.description}
          </p>
        </div>
      </div>
      <div className="mb-16">
        <h1 className="text-xl md:text-3xl font-semibold flex max-w-[90%] mx-auto mt-8">
          Similar Products
        </h1>
        {similarProduct && similarProduct?.length > 0 ? (
          ...
          <div className="flex flex-col items-center p-4">
            <div className="text-slate-300 font-extralight flex items-center justify-center gap-4 p-4">
              ...
              <h1 className="text-md md:text-xl sm:text-lg text-zinc-400">
                No similar product till now...
              </h1>
            </div>
          </div>
        )}
