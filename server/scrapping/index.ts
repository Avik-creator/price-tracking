...
export async function scrapeProduct(productUrl: string): Promise<any> {
  ...
  const port = 9222;
  const session = Math.floor(Math.random() * 10000);
  const options = {
    auth: {
  ...
  try {
    ...
    const discount = $("span.savingPriceOverride ")
      .text()
      .trim()
      .replace(/[-%]/g, "");
    ...
    const category = getCategory($(".nav-a.nav-b"));
    const data = {
      url: productUrl,
      productTitle: title,
    ...
    throw new Error(`Error in scraping product: ${e.message}`);
  }
}
